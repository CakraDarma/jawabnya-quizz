	...
	async (payload: GameSettings, { rejectWithValue }) => {
		try {
			...
			return rejectWithValue(
				'There are not enough available questions for your criteria'
			);
		} catch (rejected) {
...
const gameSlice = createSlice({
	...
	reducers: {
		answerQuestion: (
			...
			state.score +=
				action.payload.id === currentQuestion.correct_answer.id ? 1 : 0;
			...
			if (state.currentQuestionIndex === state.questions.length - 1) {
				...
				state.currectQuestionDescription =
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(
				...
				(state, action: PayloadAction<UpdatedQuestion[]>) => {
					state.questions = action.payload;
					state.currentQuestionIndex = initialState.currentQuestionIndex;

					const initialQuestion =
						state.questions[initialState.currentQuestionIndex];
			...
			.addCase(getCategories.fulfilled, (state, action) => {
				state.categories = constructCategories([
